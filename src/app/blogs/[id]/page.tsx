...
export default async function BlogDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mainImagePath = post.image ? `/uploads/blogs/${post.image}` : null;

  return (
    <main>
      <section className="page-header-bg py-16 text-white text-center">
        <div className="container mx-auto px-4">
          <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight">{post.name}</h1>
          <nav className="flex justify-center mt-6" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-2">
              ...
              <li aria-current="page">
                <div className="flex items-center">
                  <i className="fas fa-chevron-right text-white/50 mx-2 text-xs"></i>
                  <span className="ml-1 text-sm font-medium text-white md:ml-2 truncate max-w-xs">{post.name}</span>
                </div>
              </li>
            </ol>
          </nav>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              <article className="prose lg:prose-xl max-w-none">
                {mainImagePath && (
                  <img src={mainImagePath} alt={post.name} className="w-full h-auto rounded-2xl shadow-lg mb-8" />
                )}
                ...
                <div className="flex items-center text-sm text-gray-500 mb-4">
                  ...
                  <span><i className="far fa-user mr-2"></i>By {post.author_name || 'Admin'}</span>
                </div>

                <h1 className="text-3xl font-bold mb-6">{post.name}</h1>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border">
                  ...
                  <div className="space-y-4">
                    {recentPosts.map(recent => {
                      const img = recent.image ? `/uploads/blogs/${recent.image}` : 'https://placehold.co/100x70/f0f0f0/ccc?text=Image';
                      return (
                        <Link key={recent.id} href={`/blogs/${recent.id}`} className="flex items-center group space-x-4">
                          <img src={img} alt={recent.name} className="w-20 h-16 object-cover rounded-lg flex-shrink-0" />
                          <div>
                            <h4 className="font-semibold text-sm leading-tight group-hover:text-orange-600 transition-colors">{recent.name}</h4>
